...
class ConfigBeanDefinitionParser implements BeanDefinitionParser {
	...
	private AbstractBeanDefinition parseDeclareParents(Element declareParentsElement, ParserContext parserContext) {
		// <1> 创建 DeclareParentsAdvisor 类型的 BeanDefinition 构建器
		BeanDefinitionBuilder builder = BeanDefinitionBuilder.rootBeanDefinition(DeclareParentsAdvisor.class);
		// <2> 添加 `implement-interface`、`types-matching` 构造器入参
		builder.addConstructorArgValue(declareParentsElement.getAttribute(IMPLEMENT_INTERFACE));
		builder.addConstructorArgValue(declareParentsElement.getAttribute(TYPE_PATTERN));

		// <3> 获取 `default-impl`、`delegate-ref` 配置
		String defaultImpl = declareParentsElement.getAttribute(DEFAULT_IMPL);
		String delegateRef = declareParentsElement.getAttribute(DELEGATE_REF);

		// <4> `default-impl` 和 `delegate-ref` 只能配置其中一个，然后取其作为构造器入参
		...
		// <5> 通过构造器构建一个 RootBeanDefinition 对象
 		AbstractBeanDefinition definition = builder.getBeanDefinition();
		// <6> 设置来源
		definition.setSource(parserContext.extractSource(declareParentsElement));
		// <7> 注册上面创建的 RootBeanDefinition，自动生成名称
		parserContext.getReaderContext().registerWithGeneratedName(definition);
		// <8> 返回注册的 RootBeanDefinition 对象
		return definition;
	}
	...
	private Object parsePointcutProperty(Element element, ParserContext parserContext) {
		// <1> 进行检验，`pointcut` 和 `pointcut-ref` 不能同时配置
		...
		// <2> 否则，如果有 `pointcut` 配置，就是 Pointcut 的表达式
		else if (element.hasAttribute(POINTCUT)) {
			// Create a pointcut for the anonymous pc and register it.
			// <2.1> 获取表达式
			String expression = element.getAttribute(POINTCUT);
			// <2.2> 根据表达式创建一个 AspectJExpressionPointcut
			AbstractBeanDefinition pointcutDefinition = createPointcutDefinition(expression);
			// <2.3> 设置来源
			pointcutDefinition.setSource(parserContext.extractSource(element));
			// <2.4> 返回创建的 RootBeanDefinition
			return pointcutDefinition;
		}
		// <3> 否则，如果有 `pointcut-ref` 配置，也就是引用的一个 Pointcut
